...
public class XImage {
    ...
    public static Image getApplcon() {
        URL url = XImage.class.getResource("/img/BẢNG MẮT COUTURE COLOR CLUTCH.jpg");
    ...
    public static void saveAvatar(File src) {
        File newFile = new File("ImgAvatar", src.getName());
        if (!newFile.getParentFile().exists()) {
            newFile.getParentFile().mkdir();
        }
        try {
